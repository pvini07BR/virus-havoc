...
onready var goBackButton = $BigAssWrapper/gunSelectionArea/centralizer/buttons/VBoxContainer2/GoBack
onready var equipGunButton = $BigAssWrapper/gunSelectionArea/centralizer/buttons/EquipGun
onready var previewSlot1 = $BigAssWrapper/gunSelectionArea/centralizer/buttons/VBoxContainer/EquippedGunsPreview/slot1
onready var previewSlot2 = $BigAssWrapper/gunSelectionArea/centralizer/buttons/VBoxContainer/EquippedGunsPreview/slot2
onready var touchButtons = $BigAssWrapper/gunSelectionArea/centralizer/buttons/VBoxContainer/touchButtons
...
func _process(delta):
	if isSubAGun == true and !isSubAGunOnce:
		...
		if GameManager.platform == GameManager.PLATFORMS.MOBILE:
			...
			touchButtons.get_node("Button").disabled = false
			touchButtons.get_node("left/switchGunLeftButton").action = "ui_selectWeapon0"
			touchButtons.get_node("right/switchGunLeftButton").action = "ui_selectWeapon1"
		for i in GameManager.guns.size():
	...
	if !isSubAGun:
		...
		touchButtons.get_node("left/switchGunLeftButton").action = ""
		touchButtons.get_node("right/switchGunLeftButton").action = ""
		touchButtons.get_node("Button").disabled = true
		for i in GameManager.guns.size():
...
func _on_GunList_item_selected(index):
	if !isSubAGun:
		...
		if instanciatedGuns[index].ID <= 9:
			gunID.text = ("#00%d" % instanciatedGuns[index].ID)
		elif instanciatedGuns[index].ID >= 10:
			gunID.text = ("#0%d" % instanciatedGuns[index].ID)
		elif instanciatedGuns[index].ID >= 100:
			gunID.text = ("#%d" % instanciatedGuns[index].ID)
