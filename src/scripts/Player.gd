...
func _process(delta): # Essa função executa coisas dentro a todo frame. O argumento "delta" é a framerate/velocidade do jogo.
	var rotacao = move.x * 0.03 # Ele pega o move.x e muliplica por 0.2
	$col.rotation_degrees = lerp($col.rotation_degrees, rotacao, 0.03) # Isso fará uma interpolação que fará o nó girar de acordo com a var rotação.
